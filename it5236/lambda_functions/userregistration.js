	...
	if(errors.length > 0) {
		...
		conn.connect(function(err) {
			...
			if (err)  {
				...
				var sql = "SELECT registrationcode FROM userregistrations WHERE userid = ?"; // ? is a placeholder
				...
				conn.query(sql, [event.userid], function (err, result) {
				  	if (err) {
						...
						callback(null, json);
						setTimeout(function(){conn.end(); }, 3000); // waits 3 seconds before ending connection, giving callback enough to execut, recommended by Jonathan
				  	}
