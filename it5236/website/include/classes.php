	...
	*/
    ...
    public function register($username, $password, $email, $registrationcode, &$errors) {
        ...
        if (sizeof($errors) == 0) {
			...
			$ch = curl_init(); //curl_init = initialize a cURL object
			curl_setopt($ch, CURLOPT_URL, $url); // setopt = sets an option for the cURL handles
			...
			curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			$response  = curl_exec($ch); //curl_exec = Performs a cURL session
			$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
;
			...
			if ($response === FALSE) {
				...
				if($httpCode == 400) {
					...
					if (sizeof($errors) == 0) {
						$errors[] = "Bad input";
//$this->auditlog("register", json_decode($response));
					}
